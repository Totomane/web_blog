...
<div id="content" class="fade-in bg-black pb-24 min-h-[50vh] pt-4">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-1 p-2">
        <?php if(empty($projects)): ?>
            <div class="col-span-full py-24 text-center text-neutral-500 font-light tracking-widest uppercase text-sm border border-neutral-800 mx-6 md:mx-0">
                ...
                <div class="relative group fade-in block">
                    <a href="index.php?action=project&id=<?= $project['id'] ?>" class="block w-full h-full">
                        <img src="<?= htmlspecialchars($project['main_image_path'] ?? '') ?>"
                            class="w-full h-[65vh] object-cover transition duration-700 group-hover:scale-105 group-hover:grayscale-0 grayscale"
                            alt="<?= htmlspecialchars($project['title']) ?>">
                        <div class="absolute bottom-0 w-full bg-black bg-opacity-50 text-center py-3">
                            <h2 class="uppercase text-lg tracking-widest text-white"><?= htmlspecialchars($project['title']) ?></h2>
                        </div>
                    </a>
                    <?php if (!empty($_SESSION['is_admin'])): ?>
                        <form action="index.php?action=delete_project" method="POST" class="absolute top-4 right-4 z-20" onclick="event.stopPropagation(); event.preventDefault();"
                            ...
                            <button type="submit" onclick="event.stopPropagation();" class="text-white/30 hover:text-white transition-colors duration-300 p-2">
        ...
        <?php if (!empty($_SESSION['is_admin'])): ?>
            <div class="relative group fade-in cursor-pointer" onclick="openCreateModal()">
                <div
                    class="w-full h-[80vh] bg-neutral-900 border-2 border-dashed border-neutral-700 flex items-center justify-center hover:bg-neutral-800 transition duration-700">
                    <div class="text-center">
                        <div class="text-6xl text-neutral-500 mb-4">+</div>
                        <h2 class="uppercase text-lg tracking-widest text-neutral-500">Ajouter un projet</h2>
                    </div>
                </div>
            </div>
        <?php endif; ?>
                ...
                <form action="index.php?action=create" method="post" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" required
                            class="w-full bg-neutral-800/50 border border-neutral-700 text-white px-4 py-3 h-32 focus:outline-none focus:border-white focus:bg-neutral-800 transition duration-300 resize-none placeholder-neutral-600"
                            ></textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="location" name="location" required
                            class="w-full bg-neutral-800/50 border border-neutral-700 text-white px-4 py-3 focus:outline-none focus:border-white focus:bg-neutral-800 transition duration-300 placeholder-neutral-600"
                            >
                    </div>
                    ...
                    <div>
                        ...
                        <select id="category" name="category"
                            class="w-full bg-neutral-800/50 border border-neutral-700 text-white px-4 py-3 focus:outline-none focus:border-white focus:bg-neutral-800 transition duration-300">
                    ...
                    <div>
                        <label for="main_image"
                            ...
                            principale</label>
                        <label
                            class="flex flex-col items-center justify-center w-full h-32 border-2 border-neutral-700 border-dashed cursor-pointer hover:bg-neutral-800/50 hover:border-neutral-500 transition duration-300">
